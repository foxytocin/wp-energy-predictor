...
class WPDataCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self,hass,source):
        super().__init__(hass, "wp_energy_predictor", update_interval=timedelta(seconds=UPDATE_INTERVAL))
        self.source=source
    ...
    async def _async_update_data(self):
        ...
        def get_stats(start,end):
            return statistics_during_period(
                self.hass,start,end,"day",[self.source],["change"],True
            )
        ...
        for m in range(1,13):
            if m<dt.month:
                ...
                if pst and self.source in pst:
                    v=float(pst[self.source][0]["change"])
                months[m]=v
            elif m==dt.month:
